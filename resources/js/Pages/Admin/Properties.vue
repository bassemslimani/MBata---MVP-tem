...
const statuses = [
    { value: 'pending', label: 'admin.users.statuses.pending' },
    { value: 'approved', label: 'admin.shared.status_badge.approved' },
    { value: 'rejected', label: 'admin.shared.status_badge.rejected' },
    { value: 'active', label: 'admin.shared.status_badge.active' },
    { value: 'inactive', label: 'admin.shared.status_badge.inactive' }
]
...
const applyFilters = () => {
    ...
    if (wilayaFilter.value) params.append('wilaya', wilayaFilter.value)

    router.get(route('admin.properties'), Object.fromEntries(params), {
...
const approveProperty = (propertyId) => {
    loading.value = true
    router.post(route('admin.properties.approve', propertyId), {}, {
        onSuccess: () => {
            loading.value = false
        }
    })
...
const rejectProperty = (propertyId) => {
    loading.value = true
    router.post(route('admin.properties.reject', propertyId), {}, {
        onSuccess: () => {
            loading.value = false
        }
    })
...
<template>
    <AdminLayout>
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900">{{ $t('admin.properties.title') }}</h1>
            <p class="text-gray-600 mt-1">{{ $t('admin.properties.subtitle') }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        {{ $t('admin.properties.filter_status') }}
                    </label>
                    <select
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        {{ $t('admin.properties.table.verified') }}
                    </label>
                    <select
                    ...
                    >
                        <option value="">{{ $t('common.any') }}</option>
                        <option value="1">{{ $t('admin.shared.status_badge.verified') }}</option>
                        <option value="0">{{ $t('admin.properties.statuses.pending') }}</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    >
                        <option value="">{{ $t('common.any') }}</option>
                        <option v-for="wilaya in wilayas" :key="wilaya.id" :value="wilaya.code">
                            {{ wilaya.name }}
                        </option>
                    </select>
                </div>
